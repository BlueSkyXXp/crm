...
import com.stone.system.domain.SysDeptSimple;
...
public class SysUserController extends BaseController {
    ...
    @Autowired
    private ISt02CrmTeamManageService teamManageService;
     ...
     * 获取用户列表
     */
    @ApiOperation("获取部门基本信息列表")
    @PreAuthorize("@ss.hasPermi('system:user:list')")
    @GetMapping("/list/deptSimple")
    public TableDataInfo deptList() {
        startPage();
        List<SysDeptSimple> list = userService.selectDeptSimple();
        return getDataTable(list);
    }
     ...
     * 获取用户列表
     */
    @ApiOperation("获取用户基本信息列表")
    @PreAuthorize("@ss.hasPermi('system:user:list')")
    @GetMapping("/list/userSimple")
    public TableDataInfo userList() {
        startPage();
        List<SysUserSimple> list = userService.selectUserSimple();
        return getDataTable(list);
    }
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user) {
        ...
        St02CrmTeamManage teamManage = new St02CrmTeamManage();
        teamManage.setUserId(user.getUserId().toString());
        teamManage.setOperatorId(SecurityUtils.getUserId());
        teamManage.setReceiveNewData(0L);
        teamManage.setRemark("init");
        teamManageService.insertSt02CrmTeamManage(teamManage);
        return toAjax(rows);
    }
    ...
    public AjaxResult changeStatus(@RequestBody SysUser user) {
        ...
        if ("1".equals(user.getStatus())) {
            ...
            customerService.updateSt02CrmCustomer(customer);
        }
    ...
    public String getTreeDeptName(Long userId) {
        SysUser user = userService.selectUserById(userId);
        String newDeptName = "";
        if (user != null) {
            ...
            // ancestors = "100,101,102,103"  101可以忽略， 后面的才是部门id
            String[] deptIds = ancestors.split(",");
            // 101 忽略， 后面更具id 查询部门名称 通过连接符号-连接起来

            for (int i = 1; i < deptIds.length; i++) {

                String deptName = deptService.selectDeptById(Long.valueOf(deptIds[i])).getDeptName();
                if (deptName.length() > 0) {
                    newDeptName += deptName + "-";
                }
            }
            newDeptName += dept.getDeptName();
        }
        return newDeptName;
    }
}
