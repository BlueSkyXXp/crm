<template>
  ...
  >
    ...
    <template v-for="(configItem, configItemIndex) in computedConfig.item">
      ...
      <el-table
        v-else-if="configItem.xType === 'table'"
        :key="configItemIndex"
        :data="formData[configItem.table.name]"
        :height="configItem.table.height"
        :max-height="configItem.table.maxHeight"
        :stripe="configItem.table.stripe"
        :border="true"
        :size="configItem.table.size"
        :fit="configItem.table.fit"
        :show-header="configItem.table.showHeader"
        :highlight-current-row="configItem.table.highlightCurrentRow"
        :current-row-key="configItem.table.currentRowKey"
        :row-class-name="configItem.table.rowClassName"
        :row-style="configItem.table.rowStyle"
        :cell-class-name="configItem.table.cellClassName"
        :cell-style="configItem.table.cellStyle"
        :header-row-class-name="configItem.table.headerRowClassName"
        :header-row-style="configItem.table.headerRowStyle"
        :header-cell-class-name="configItem.table.headerCellClassName"
        :header-cell-style="configItem.table.headerCellStyle"
        :row-key="configItem.table.rowKey"
        :empty-text="configItem.table.emptyText"
        :default-expand-all="configItem.table.defaultExpandAll"
        :expand-row-keys="configItem.table.expandRowKeys"
        :default-sort="configItem.table.defaultSort"
        :tooltip-effect="configItem.table.tooltipEffect"
        :show-summary="configItem.table.showSummary"
        :sum-text="configItem.table.sumText"
        :summary-method="configItem.table.summaryMethod"
        :span-method="configItem.table.spanMethod"
        :select-on-indeterminate="configItem.table.selectOnIndeterminate"
        :indent="configItem.table.indent"
        :lazy="configItem.table.lazy"
        :load="configItem.table.load"
        :tree-props="configItem.table.treeProps"

        @select="(a, b) => computeFunction(configItem.table.select, a, b) "
        @select-all="(a) => computeFunction(configItem.table.selectAll, a) "
        @selection-change="(a) => computeFunction(configItem.table.selectionChange, a)"
        @cell-mouse-enter="(a, b, c, d) => computeFunction(configItem.table.cellMouseEnter, a, b, c, d) "
        @cell-mouse-leave="(a, b, c, d) => computeFunction(configItem.table.cellMouseLeave, a, b, c, d) "
        @cell-click="(a, b, c, d) => computeFunction(configItem.table.cellClick, a, b, c, d) "
        @cell-dblclick="(a, b, c, d) => computeFunction(configItem.table.cellDblclick, a, b, c, d) "
        @row-click="(a, b, c) => computeFunction(configItem.table.rowClick, a, b, c) "
        @row-contextmenu="(a, b, c) => computeFunction(configItem.table.rowContextmenu, a, b, c) "
        @row-dblclick="(a, b, c) => computeFunction(configItem.table.rowDblclick, a, b, c) "
        @header-click="(a, b) => computeFunction(configItem.table.headerClick, a, b) "
        @header-contextmenu="(a, b) => computeFunction(configItem.table.headerContextmenu, a, b) "
        @sort-change="(a) => computeFunction(configItem.table.sortChange, a) "
        @filter-change="(a) => computeFunction(configItem.table.filterChange, a) "
        @current-change="(a, b) => computeFunction(configItem.table.currentChange, a, b) "
        @header-dragend="(a, b, c, d) => computeFunction(configItem.table.headerDragend, a, b, c, d) "
        @expand-change="(a, b) => computeFunction(configItem.table.expandChange, a, b) "
      >
        <div slot="empty">
          <el-empty :image="noData" :image-size="100" />
        </div>
        <template v-for="columnConfig in configItem.table.column">
          <xColumn :key="columnConfig.name" :config="columnConfig" :tab-config="configItem.table">
            <!-- slot 传递 -->
        ...
        <el-table-column
          v-if="configItem.table.operate.show"
          :label="configItem.table.operate.label"
          :width="configItem.table.operate.width"
          :header-align="configItem.table.operate.headerAlign"
          :align="configItem.table.operate.align"
        >
          <template #default="scope">
            <el-form-item label-width="0px">
              <template v-for="(btn, btnIndex) in configItem.table.operate.btn">
                <el-button
                  ...
                  @click="btn.click(formData[configItem.table.name], scope.$index, configItem.table.remove)"
                >
          ...
          >
            ...
            <xForm v-if="tabConfig.type === 'form'" :ref="`${tabConfig.name}_XForm`" v-model="formData[tabConfig.name]" :config="childFormConfig(tabConfig.formConfig)">
              <template v-for="tabFormItem in tabConfig.formConfig.item">
                ...
                <slot v-else-if="typeof tabFormItem.slot === 'string' && computeBoolen(tabFormItem.show, true)" :slot="tabFormItem.slot" :name="tabFormItem.slot" />
              </template>
            </xForm>
            <el-table
              v-else
