    ...
    <div slot="search">
      ...
      <div class="selectPerson bg-blue m10 p10">
        ...
        <el-radio-group v-model="checkUserId">
          <el-radio v-for="item in users" :label="item.userId">{{ item.nickName || item.userName }}</el-radio>
        </el-radio-group>
...
<script>
import { assignNewCustomer } from "@/api/crm/customer";
import { listCustomer, getCustomer, delCustomer, addCustomer, updateCustomer } from "@/api/crm/customernew";
...
export default {
  ...
  methods: {
    ...
    /** 分配按钮操作 */
    handleAssign() {
      assignNewCustomer({ userId: this.checkUserId, customerIds: this.ids }).then((response) => {
