<template>
  <table-panel title=" 我的客户">
      ...
      <el-form v-show="showSearch" ref="queryForm" :model="queryParams" label-position="right" :inline="true" label-width="150px">
        ...
        <el-form-item label="贷款条件" class="flex">
          <div class="flex">
            ...
            <el-select v-if="loanValue === 'carInfo'" v-model="queryParams.carInfo" placeholder="请选择" style="width: 140px" clearable>
              <el-option
            ...
            <el-select v-if="loanValue === 'propertyInfo'" v-model="queryParams.propertyInfo" placeholder="请选择" style="width: 140px" clearable>
              <el-option
                v-for="(item, index) in hasOptions"
                :key="index"
                :label="item.label"
            ...
            <el-select v-if="loanValue === 'policyInfo'" v-model="queryParams.policyInfo" placeholder="请选择" style="width: 140px" clearable>
              <el-option
                v-for="(item, index) in hasOptions"
                :key="index"
                :label="item.label"
            ...
            <el-select v-if="loanValue === 'socialSecurity'" v-model="queryParams.socialSecurity" placeholder="请选择" style="width: 140px" clearable>
              <el-option
                v-for="(item, index) in hasOptions"
                :key="index"
                :label="item.label"
            ...
            <el-select v-if="loanValue === 'accFund'" v-model="queryParams.accFund" placeholder="请选择" style="width: 140px" clearable>
              <el-option
                v-for="(item, index) in hasOptions"
                :key="index"
                :label="item.label"
    ...
    <div slot="btm">
      <el-button v-hasPermi="['crm:customer:add']" type="primary" plain icon="el-icon-plus" @click="handleAdd">新增</el-button>
      <el-button
...
<!--      >删除</el-button>-->
      <el-button
        ...
        @click="addToPool"
      >加入公共池</el-button>
      <el-button v-hasPermi="['crm:customer:export']" type="warning" plain icon="el-icon-download" @click="handleExport">导出</el-button>
    ...
    <div slot="content">
      ...
      <st-dialog v-if="open" :title="title" height="650px" width="90%" append-to-body @close="cancel" @save="submitForm">
        <el-form ref="form" :model="form" :rules="rules" label-width="120px">
          ...
          <el-row>
            ...
            <el-col :span="7">
              <el-form-item label="客户星级：" prop="level">
                <el-select v-model="form.level" placeholder="请选择星级" style="display: block">
                  <el-option
                    v-for="(item, index) in dict.type.crm_customer_star"
                    :key="index"
                    :label="item.label"
          ...
          <template v-if="form.professional">
            <el-row>
              <el-col :span="7">
                <el-form-item label="单位性质：" prop="propertyStatus">
                  <el-select v-model="form.level" placeholder="请选择单位性质" style="display: block">
                    <el-option
                      v-for="(item, index) in dict.type.crm_customer_professional_type"
                      :key="index"
                      :label="item.label"
                      :value="parseInt(item.value)"
                      :disabled="item.disabled"
              ...
              <el-col :span="7">
                <el-form-item label="工资：" prop="area">
                  <el-input v-model="form.area" placeholder="请输入面积" />
                </el-form-item>
              </el-col>
              <el-col :span="7">
                <el-form-item label="发薪方式：" prop="houseCity">
                  <el-select v-model="form.level" placeholder="请选择发薪方式" style="display: block">
                    <el-option
                      v-for="(item, index) in dict.type.crm_customer_pay_type"
                      :key="index"
                      :label="item.label"
                      :value="parseInt(item.value)"
                      :disabled="item.disabled"
      ...
      <detail v-if="showDetail" ref="detailRef" :params="detailParams" @close="close" />
    </div>
    <div slot="footer">
...
export default {
  ...
  dicts: [
    "crm_customer_follow_times",
    "crm_customer_star",
    "crm_loan_type",
    "crm_customer_status",
    "crm_customer_professional_type",
    "crm_customer_pay_type",
    "sys_user_sex",
    "crm_has_not"
  ],
  data() {
  ...
  methods: {
    ...
    addToPool(row) {
      ...
      this.$modal
        .confirm('是否确认将编号为"' + customerIds + '"的客户加入公共池？')
        .then(function() {
          console.log("customerIds", customerIds);
          return publicCustomer({customerIds:customerIds});
        })
        .then(() => {
          this.getList();
          this.$modal.msgSuccess("加入公共池成功");
        })
        .catch(() => {});
    },
    /** 提交按钮 */
