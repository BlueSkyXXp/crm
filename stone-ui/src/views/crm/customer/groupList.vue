    ...
    <div slot="search">
      <el-form v-show="showSearch" ref="queryForm" :model="queryParams" label-position="right" :inline="true" label-width="150px">
        ...
        <el-form-item label="客户" prop="customerId">
          <div class="flex">
            <el-select v-model="customerValue" placeholder="请选择" style="width: 110px">
              <el-option
        ...
        <el-form-item label="贷款类型" prop="loanType" clearable>
          <el-select v-model="queryParams.loanType" placeholder="请选择贷款类型" style="width: 240px">
            <el-option
        ...
        <!-- <el-form-item label="部门" prop="dept">
          ...
          :style="{width: '100%'}" placeholder="请选择部门" clearable @change="userQueryOptions"></el-cascader>
        </el-form-item> -->
        <el-form-item label="顾问" prop="params">
          <el-select v-model="queryParams.userId" placeholder="请选择" style="width: 130px" clearable>
            <el-option v-for="(item, index) in usersOptions" :key="index" :label="item.userName" :value="item.userId" />
          </el-select>
        </el-form-item>
...
<!--        <el-table-column label="最近备注时间" align="center" prop="lastRemarkTime" />-->
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listGroupCustomer(this.queryParams).then((response) => {
    ...
    userQueryOptions() {
      if (this.depts.length == 0) {
        this.depts = [this.deptId];
      }
      deptUser(this.depts[this.depts.length - 1]).then((response) => {
        this.usersOptions = response.rows;
      });
    },
    // 取消按钮
    cancel() {
